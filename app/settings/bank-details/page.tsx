    ...
    const fetchData = async () => {
      try {
        ...
        if (user?.bank_details) {
          try {
            const details = await paymentsService.getBankDetails(
              user.bank_details,
            );
            ...
            showToast.error(
              "Failed to load your bank details. Please try again.",
            );
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const bankData = {
        ...
        bank_name:
          banks.find((bank) => bank.id.toString() === formData.bank_name)
      ...
      if (bankDetails?.id) {
        ...
        response = await paymentsService.updateBankDetails(
          bankDetails.id,
          bankData,
        );
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#FBFBFB] w-full flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#FCC01C]" />
      </div>
    );
  }
