...
import {
  MenuSelection,
  QuoteItem as MenuQuoteItem,
} from "@/components/quotes/menu-selection";
...
const Page: React.FC = () => {
  ...
  const firstImageRel: string | null =
    Array.isArray(quote?.images) && quote.images.length > 0
      ? quote.images[0]?.image
      : null;
  const imageSrc = firstImageRel
    ? firstImageRel.startsWith("http")
      ? firstImageRel
      : `${apiBase}${firstImageRel}`
  ...
  const personName =
    (isHost ? chefName : hostName) || (isHost ? "Chef" : "Host");
  ...
  const breakdown = (() => {
    ...
    const total =
      typeof totalNum === "number" && !Number.isNaN(totalNum) ? totalNum : 0;
    ...
    const pricePerPersonFromBooking =
      typeof pppRaw === "string" || typeof pppRaw === "number"
        ? Number(pppRaw)
        : undefined;

    let guests: number | undefined =
      typeof guestsRaw === "number" ? guestsRaw : undefined;
    let pricePerPerson: number | undefined = pricePerPersonFromBooking;

    if (
      typeof guests === "number" &&
      (pricePerPerson === undefined || Number.isNaN(pricePerPerson))
    ) {
      // derive price per person if we have guests
      pricePerPerson = guests > 0 ? subtotal / guests : 0;
    } else if (
      (guests === undefined || Number.isNaN(guests)) &&
  ...
  const onPayQuote = async () => {
    ...
    try {
      ...
      const bookingId =
        booking?.id ||
        (quote as any)?.booking_id ||
        (quote as any)?.booking?.id;
