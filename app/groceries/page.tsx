...
const ChefGroceriesPage: React.FC = () => {
  ...
  const [currentStatus, setCurrentStatus] = useState<(typeof statuses)[number]>("Active");

  // Items modal state
  const [itemsOpen, setItemsOpen] = useState(false);
  const [modalTitle, setModalTitle] = useState("");
  const [modalItems, setModalItems] = useState<{ name: string; weight: string; measurement_unit: string }[]>([]);

  const currencySymbol = getCurrencySymbol({ currency: user?.currency, country: user?.country });
  ...
  const fetchGroceries = async () => {
    ...
    setError(null);
    try {
      const res = await groceriesService.getGroceries({ status: currentStatus, page: 1, page_size: 20 });
      const results = res.results || [];
      if (typeof window !== "undefined" && process.env.NODE_ENV !== "production") {
        // Log a couple of items to inspect image payload shape
        console.log("[Groceries] fetched", results.length, "items. Sample images:",
  ...
  return (
    <div className="w-full min-h-screen relative bg-[#FBFBFB]">
        ...
        <section className="flex justify-center mt-6">
          <div className="w-full max-w-[1114px] px-6">
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {loading && <p className="text-sm text-gray-500">Loading groceries...</p>}
              {error && !loading && <p className="text-sm text-red-600">{error}</p>}
              {!loading && !error && filteredGroceries.length === 0 && (
                <div className="text-sm text-gray-500">No groceries found.</div>
              )}
              {!loading && !error && filteredGroceries.length > 0 && filteredGroceries.map((g) => {
                const isMultiple = Array.isArray(g.items) && g.items.length > 0;
                const firstImageRaw = extractImageUrl(g.images?.[0]);
                const firstImage = resolveImageUrl(firstImageRaw) || "https://via.placeholder.com/300x200?text=Grocery";
      ...
      <ItemsModal open={itemsOpen} onClose={() => setItemsOpen(false)} title={modalTitle} items={modalItems} />
    </div>
  );
};
