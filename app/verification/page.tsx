...
const DocumentUploadModal = ({ open, onClose, onUploadSuccess }: DocumentUploadModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950 bg-opacity-40">
      <dialog open className="relative w-[454px] max-w-[95vw] bg-white rounded-lg p-0 shadow-lg">
        ...
        <form className="flex flex-col gap-6 px-6 py-6">
          <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-[#CFCFCE] rounded-lg p-6 cursor-pointer w-full min-h-[120px]">
            <div className="flex flex-col items-center">
              <div className="w-10 h-10 flex items-center justify-center rounded-full bg-[#F5F5F5] mb-2">
                <svg width="24" height="24" fill="none" stroke="#FCC01C" strokeWidth="2" viewBox="0 0 24 24">
                  <circle cx="12" cy="12" r="11" stroke="#FCC01C" strokeWidth="2" fill="none" />
                  <path d="M12 8v8M8 12h8" stroke="#FCC01C" strokeWidth="2" strokeLinecap="round" />
                </svg>
              </div>
              <span className="text-sm text-black font-medium text-center">Click here to choose a file</span>
              <span className="text-xs text-gray-500 text-center">Max file size is 2mb.</span>
            </div>
          ...
          {file && (
            <div className="flex items-center justify-between bg-gray-50 rounded px-3 py-2 mt-2">
              ...
              <button type="button" onClick={handleRemove} className="ml-4 text-gray-400 hover:text-red-500" aria-label="Remove file">
          ...
          <button type="button" onClick={handleUpload} disabled={!file || uploading} className="flex gap-2 justify-center items-center px-28 py-3 h-12 bg-amber-400 rounded-lg border border-amber-400 border-solid shadow-sm w-full max-sm:px-5 max-sm:py-3 max-sm:w-full text-base font-bold leading-6 text-white disabled:opacity-60">
...
const IdentityVerificationModal = ({ open, onClose }: IdentityVerificationModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!method || !idNumber) {
      showToast.error("Please select a verification type and enter your ID number.");
      return;
    }
    if (!user?.email) {
    ...
    try {
      ...
      const verified = (data && data.data && typeof data.data.identity_verified === "boolean")
      ...
      <dialog open className="relative w-[454px] max-w-[95vw] bg-white rounded-lg p-0 shadow-lg">
        ...
        <form className="flex flex-col gap-4 px-6 py-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-end gap-2 mt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm rounded-md border border-gray-300">
              Cancel
            </button>
            <button type="submit" disabled={submitting} className="px-4 py-2 text-sm rounded-md bg-amber-400 text-white hover:bg-amber-500 disabled:opacity-60">
...
const VerificationPage = () => {
  ...
  const [selectedOption, setSelectedOption] = useState<"document" | "identity" | null>(null);
  ...
  return (
    <main className="flex flex-col items-center px-4 py-8 max-w-4xl mx-auto">
      ...
      <div className="w-full max-w-[653px] mb-12">
        <h1 className="text-black text-2xl font-bold leading-8 mb-4 max-sm:text-xl">Verification</h1>
      </div>

      <form className="w-full max-w-[653px] space-y-6">
        <div className={`border w-full px-4 py-3 rounded-md border-solid border-[#CFCFCE] max-sm:p-3 flex items-start gap-3 justify-between ${identityVerified ? "opacity-60" : ""}`}>
          <div>
            <h3 className="text-[#3F3E3D] text-[15px] font-medium mb-1">Verify your identity</h3>
            <p className="text-[rgba(50,51,53,0.5)] text-xs font-normal">Provide an official ID to verify your identity</p>
          </div>
          {identityVerified ? (
        ...
        {isChef && (
          <div className={`border w-full px-4 py-3 rounded-md border-solid border-[#CFCFCE] max-sm:p-3 flex items-start gap-3 justify-between ${!identityVerified && !hasCertificate ? "opacity-90" : ""}`}>
            <div>
              ...
              <p className="text-[rgba(50,51,53,0.5)] text-xs font-normal">Upload your Culinary school certificate here</p>
            </div>
            {hasCertificate ? (
              isVerified ? (
                <span className="inline-flex items-center gap-1 mt-0.5 px-2 py-1 text-[11px] font-medium rounded-full bg-green-100 text-green-700 border border-green-200">
                ...
                <span className="inline-flex items-center gap-1 mt-0.5 px-2 py-1 text-[11px] font-medium rounded-full bg-amber-100 text-amber-800 border border-amber-200">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 9v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" stroke="#92400E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  Under Review
                </span>
              )
      ...
      <DocumentUploadModal open={showModal} onClose={() => setShowModal(false)} onUploadSuccess={handleUploadSuccess} />
      <IdentityVerificationModal open={showIdentityModal} onClose={() => setShowIdentityModal(false)} />
    </main>
  );
};
