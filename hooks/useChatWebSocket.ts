...
export const useChatWebSocket = (chatId: number | null, accessToken: string | null) => {
  ...
  const addMessageHandler = useCallback((handler: (msg: MessageType) => void) => {
  ...
  useEffect(() => {
    const handleMessage = (message: Message) => {
      ...
      setMessages(prev => {
        ...
        console.log('Updated messages after WebSocket message:', updatedMessages);
    ...
    const fetchInitialMessages = async () => {
      ...
      try {
        ...
        setMessages(prev => {
          ...
          const newMessages = response.results.filter(msg => !existingIds.has(msg.id));
