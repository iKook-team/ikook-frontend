...
const MealDetailsForm3: React.FC<MealDetailsForm3Props> = ({ onNext, onBack }) => {
    ...
    <main className="w-[655px] h-[852px] absolute left-[393px] top-[177px]">
      ...
      <section className="absolute left-5 top-[307px] w-[613px]">
        ...
        <form className="flex flex-col flex-1 w-full" onSubmit={e => { e.preventDefault(); handleContinue(); }}>
          <label className="text-[#344054] text-sm font-medium leading-none mb-2">Meal Type</label>
            ...
            {['Breakfast', 'Lunch', 'Dinner'].map((option) => (
              <div key={option} className="flex items-center">
                <input
                  ...
                  onChange={() => handleInputChange('mealType', option)}
                  className="h-4 w-4 text-amber-500 border-gray-300 focus:ring-amber-500"
                />
                <label htmlFor={`mealType-${option}`} className="ml-2 text-base text-[#101828] font-normal cursor-pointer">
                  {option}
                </label>
              </div>
            ))}
      ...
      <div className="absolute left-[357px] top-[772px]">
        <ActionButtons
          ...
          continueDisabled={!formData.mealType || formData.preferredCuisines.length === 0}
        />
      </div>
    </main>
  );
