...
const ClassDetailsForm3: React.FC<ClassDetailsForm3Props> = ({ onNext, onBack, initialValues = {} }) => {
  ...
  const handleInputChange = (field: keyof ClassDetailsForm3Data, value: any) => {
    ...
    <main className="w-[655px] h-[852px] absolute left-[393px] top-[177px]">
      ...
      <section className="absolute left-5 top-[307px] w-[613px]">
        ...
        <form className="flex flex-col flex-1 w-full" onSubmit={e => { e.preventDefault(); handleContinue(); }}>
          <label htmlFor="startDate" className="text-[#344054] text-sm font-medium mb-2">Start date</label>
          <input
            type="date"
            id="startDate"
            name="startDate"
            ...
            required
          />
          <label htmlFor="endDate" className="text-[#344054] text-sm font-medium mb-2">End date</label>
          <input
            type="date"
            id="endDate"
            name="endDate"
      ...
      <div className="absolute left-[357px] top-[772px]">
        <ActionButtons
          ...
          continueDisabled={!formData.startDate || !formData.endDate || formData.cuisines.length === 0}
        />
      </div>
    </main>
  );
