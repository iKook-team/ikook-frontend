  ...
  const isStepValid =
    ...
    (isMultiple
      ? (localFormData.items || []).length > 0 &&
        (localFormData.items || []).every(
          (r) => r.item && r.weightUnit && r.weightValue,
        )
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-500">
                    {(localFormData.items || []).length} item
                    {(localFormData.items || []).length !== 1 ? "s" : ""}
                  </span>
                </div>
                <div className="space-y-4">
                  {(localFormData.items || []).map((row, idx) => (
                    <div
                      key={idx}
                      className="grid grid-cols-1 sm:grid-cols-12 gap-3 p-4 bg-gray-50 rounded-lg border border-gray-200"
                    >
                      ...
                      <div className="sm:col-span-6">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <div className="flex">
                            ...
                            >
                              {(weightOptions as unknown as string[]).map(
                                (opt) => (
                                  <option key={opt} value={opt}>
                                    {opt}
                                  </option>
