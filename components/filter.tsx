    ...
    return (
        <div className="flex flex-col space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 md:items-center">
                <h1 className="text-xl font-bold w-full md:w-1/4">{tags.find((tag) => tag.id === selectedTag)?.name}</h1>

                <div className="w-full md:w-2/4 flex justify-between items-center rounded-full border-1 py-2 px-4">
                    <div className="w-2/3 text-xs sm:text-sm">Cant find what you want? use the custom booking</div>
                    <Button radius="full" size="sm" className="bg-yellow-400 text-white">Custom Booking</Button>
                </div>
