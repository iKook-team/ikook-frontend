...
export const CreateMenuStep2: React.FC<CreateMenuStep2Props> = ({
  ...
  const [currentCourseIdx, setCurrentCourseIdx] = useState<number>(() => {
    const savedIdx =
      typeof formData.currentCourseIdx === "number"
  ...
  const currentCourse =
    courses[Math.min(currentCourseIdx, courses.length - 1)] ||
    courses[0] ||
    "Starter";
  ...
  const [menuSelection, setMenuSelection] = useState<Record<string, string>>(
    () => {
      ...
      courses.forEach((course: string) => {
        ...
        initial[course] =
          formData.coursesSelectionLimit?.[course]?.toString() || "1";
      });
  ...
  >(() => {
    ...
    courses.forEach((course: string) => {
      ...
      initial[course] =
        formData.coursesExtraChargePerPerson?.[course]?.toString() || "0";
    });
              ...
              <div className="mt-2 flex justify-end">
                <button
                  ...
                  onClick={async (e) => {
                    ...
                    if (!itemInput.name?.trim()) {
                      showToast.warning(
                        "Please enter the menu item name first.",
                      );
                    ...
                    try {
                      ...
                      const res =
                        await menuService.generateMenuItemDescription(payload);
