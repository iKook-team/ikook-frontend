...
export const fetchTimeSlots = async (startDate: Date, endDate: Date): Promise<TimeSlot[]> => {
  try {
    const response = await axios.get<TimeSlotsResponse>('/calendar/time-slots/', {
      params: {
...
const mapBookingToEvents = (timeSlot: TimeSlot, booking: any, index: number): BookingEvent[] => {
