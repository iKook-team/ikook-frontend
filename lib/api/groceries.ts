...
export const groceriesService = {
  async getGroceryById(id: string | number): Promise<Grocery> {
    const response = await apiClient.get<Wrapped<Grocery> | Grocery>(`/groceries/${id}/`);
  ...
  async createGrocery(payload: {
    ...
    const res = await apiClient.post<Wrapped<Grocery> | Grocery>("/groceries/", payload);
    const payloadResp: any = res.data;
    const data: Grocery | undefined = (payloadResp && payloadResp.data) ? payloadResp.data : payloadResp;
    return data as Grocery;
  },
  async uploadGroceryImage(payload: { image: string; grocery: number | string }): Promise<{ id: number; image: string; grocery: number }>{
    const res = await apiClient.post("/groceries/images/", payload);
    return res.data;
  },
  async uploadGroceryImageFile(file: File | Blob, groceryId: number | string): Promise<{ id: number; image: string; grocery: number }>{
  ...
  async uploadGroceryImageForm(formData: FormData): Promise<{ id: number; image: string; grocery: number }>{
  ...
  async updateGrocery(id: string | number, payload: Partial<{
    name: string;
    product_type: "Single Item" | "Multiple Items";
    weight_or_quantity?: string;
    measurement_unit?: string;
    ...
    const res = await apiClient.patch<Wrapped<Grocery> | Grocery>(`/groceries/${id}/`, payload);
    const dataAny: any = res.data;
    const data: Grocery | undefined = (dataAny && dataAny.data) ? dataAny.data : dataAny;
    return data as Grocery;
  },
};
